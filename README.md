...
/src
/AgendamentoMedico.API         ← Projeto ASP.NET Core MVC
/Controllers
HomeController.cs
MedicoController.cs
...
HorarioDisponivelService.cs
UsuarioContextService.cs
...
Funcionario\_Cliente.cs
IdentityRole\_Usuario.cs
...
/Infra
/Data
AppDbContext.cs
...
HorarioDisponivelRepository.cs
UsuarioRepository.cs
FuncionarioRepository.cs
...
/Views
/Home
AgendarConsulta.cshtml
/Medico
MedicoDashboard.cshtml
/Shared
\_Layout.cshtml
...
/css
/js
/lib        ← Bootstrap e FullCalendar via CDN, sem pacotes locais
appsettings.json
Program.cs
...
Se for usar PostgreSQL, instale o pacote `Npgsql.EntityFrameworkCore.PostgreSQL` e adapte a string:

```json
"ConnectionStrings": {
  "DefaultConnection": "Host=localhost;Port=5432;Database=AgendamentoMedicoDb;Username=postgres;Password=SuaSenha"
}
```

2. **Migrations e Atualização do Banco**
   Abra um terminal na pasta do projeto `AgendamentoMedico.API` e execute:
   ...
   Caso ainda não haja migrations, crie a inicial:
...
3. **Seed de Roles e Admin**
   ...
   * Usuário Admin “admin” com senha criptografada
     Ao executar `dotnet ef database update`, esses registros são inseridos automaticamente.
...
6. **Acessar no Navegador**
   ...
   * Para cadastrar novos clientes e médicos, use as rotas de registro ou endpoints dedicados.
...
1. **Gerar Migration Inicial** (caso não exista)
...
3. **Criar Novas Migrations**
   Sempre que alterar as entidades (`Domain/Entities`), crie e aplique nova migration:
